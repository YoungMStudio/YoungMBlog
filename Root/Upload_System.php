...
if ($_SESSION['level'] == '') {
	$attachDir= "../Public/Uploadfile";//上传文件保存路径，结尾不要带/
} else {
	$attachDir= "../..".$_SESSION['level']."/Public/Uploadfile";//上传文件保存路径，结尾不要带/
}
...
if($err==''){
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			switch($dirType)
			{
				case 1: $attachSubDir = 'day_'.date('Y-m-d'); break;
				case 2: $attachSubDir = 'month_'.date('Y-m'); break;
